...
function AdminMenu() {
  ...
  const handleSave = (e) => {
    ...
    const item = {
      ...currentItem,
      id: currentItem.id || Date.now(), // unique ID
      image:
        currentItem.imageFile ||
        (currentItem.imageName ? `images/${currentItem.imageName}` : "")
    };

    if (currentItem.id) {
      // Update
      savedItems = savedItems.map((i) => (i.id === currentItem.id ? item : i));
    } else {
      // Add new
          ...
          <Col key={item.id} md={4} className="mb-4">
            <Card>
              ...
              <Card.Body>
                ...
                <Card.Text>
                  Category: {item.category} <br />Price: ₹{item.price}
                </Card.Text>
